...
const mapping = { externalId: 'id', caseNumber: 'number', title: 'title', status: 'status', mappingVersion: '1' };
...
describe('H canonical provenance invariants', () => {
  ...
  it.each([undefined, NaN, Infinity, 1n, new Date(), { value: undefined }, [undefined], Number.MAX_SAFE_INTEGER + 1])(
    'rejects payloads that JSON would discard, round or coerce: %s',
    (value) => expect(() => sourceHash(value)).toThrow(),
  );
  ...
  it('rejects malformed collections and missing identities before checkpoint advance', () => {
    ...
    expect(() => mapExternalCaseItems([source, { number: 'missing-id' }], mapping)).toThrow(/row 1/);
...
describe('H complete-snapshot reconciliation', () => {
  const tracked = () => ({
    externalId: source.id, sourceHash: canonical().sourceHash, sourceVersion: null,
    mappingVersion: '1', internalId: '00000000-0000-4000-8000-000000000001',
  });
  ...
  it('does not treat an empty canonical link as a valid import', () => {
    expect(reconcileCases([canonical()], [{ ...tracked(), internalId: '' }])).toMatchObject({
      result: 'RED', missingInternalCount: 1,
    });
  });
  it('partitions every row exactly as the SQL RPC contract requires', () => {
    ...
    expect(result.recordedCount).toBe(result.changedCount + result.unchangedCount + result.missingSourceCount);
    expect(result.missingInternalCount).toBeLessThanOrEqual(result.changedCount + result.unchangedCount);
...
describe('H SQL page integrity', () => {
  function adapter(pageSize = 100) {
    ...
    return {
      ...
      connector: new GenericSqlReadConnector({
        key: 'fixture', select: 'select id, number, title, status from approved_view',
        caseMapping: mapping, capabilities: ['listCases'], pageSize,
      }, { query, healthCheck: vi.fn() }),
    };
  }
  it.each(['2junk', '1.5', '-1', '1e3', ' ', '9007199254740992'])('rejects malformed cursor %s', async (cursor) => {
  ...
  it('uses deterministic external-id ordering with parameterized pagination', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('order by tryggsignal_source."id"'), [100, 0]);
  });
});

describe('H webhook identity', () => {
  it('keeps logical event identity stable across mapping releases', async () => {
    const make = (mappingVersion: string) => new GenericInboundWebhookConnector({
      key: 'fixture', eventIdPath: 'id', eventTypePath: 'type', mappingVersion,
    }, async () => true);
...
function envelope(): JobEnvelope {
  return {
    jobId: '11111111-1111-4111-8111-111111111111', type: 'integration_inbound',
    tenantContext: '22222222-2222-4222-8222-222222222222',
    authorityContext: '33333333-3333-4333-8333-333333333333',
    correlationId: '44444444-4444-4444-8444-444444444444', causationId: null,
    idempotencyKey: 'integration-1', payload: { integration_event_id: 1 }, attempt: 1,
    createdAt: '2026-09-09T00:00:00Z', queue: 'integration_inbound',
  };
}
function database(status = 'RECEIVED') {
  const statements: string[] = [];
  const row = { id: '1', status, authority_id: envelope().authorityContext,
    connector_instance_id: envelope().tenantContext, attempt: 0 };
...
describe('H inbound canonical acknowledgement', () => {
  ...
  it('validates authority as well as connector before even accepting a replay', async () => {
    ...
    await expect(handleIntegrationInbound({ ...envelope(), authorityContext: null }, sql)).rejects.toThrow(PermanentJobError);
    await expect(handleIntegrationInbound({ ...envelope(), authorityContext: 'another-authority' }, sql)).rejects.toThrow(/scope/);
  });
  ...
  it('rejects rounded or oversized bigint ids before SQL', async () => {
    const { sql, statements } = database();
    await expect(handleIntegrationInbound({ ...envelope(), payload: { integration_event_id: '9223372036854775808' } }, sql)).rejects.toThrow(/valid/);
